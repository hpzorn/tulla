"""Tests for ralph.phases.discovery.pipeline – discovery_pipeline factory."""
...
from ralph.phases.discovery.d4 import D4Phase
...
class TestDiscoveryPipelineBudget:
    ...
    def test_custom_budget(self, tmp_path: Path) -> None:
        config = RalphConfig(discovery={"budget_usd": 12.5})
...
class TestDiscoveryPipelineMode:
    ...
    def test_downstream_mode(self, tmp_path: Path, config: RalphConfig) -> None:
        p = discovery_pipeline(
            claude_port=StubClaudePort(),
            work_dir=tmp_path,
            idea_id="idea-101",
